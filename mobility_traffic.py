...
from utils.tools import EarlyStopping, adjust_learning_rate, visual, save_to_csv, load_adj_from_numpy, normalize_adj_mx
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        ...
        return criterion
    def _get_adj(self,adj_path,device):
        season_path='dataset/PEMS/sd_rn_adj.npy'
        season_adj_mx = load_adj_from_numpy(season_path)
        adj_mx = normalize_adj_mx(season_adj_mx,'transition')
        season_supports = [torch.tensor(i).repeat(2,1,1).to(device) for i in adj_mx]

        trend_path='dataset/PEMS/sd_rn_adj.npy'
        trend_adj_mx = load_adj_from_numpy(trend_path)
        adj_mx = normalize_adj_mx(trend_adj_mx , 'transition')
        trend_supports = [torch.tensor(i).repeat(2,1,1).to(device) for i in adj_mx]
        self.adj=[season_supports, trend_supports]
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark,self.adj)
    ...
    def train(self, setting):
        ...
        self._get_adj(self.args.graph_path,self.device)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark,self.adj,self.device)
            ...
            self.time=self.time+time.time() - epoch_time
    ...
    def test(self, setting, test=0):
        ...
        print('Time:{},epoch:{}'.format(self.time,self.epoch))
